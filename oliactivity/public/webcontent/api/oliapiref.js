	...
	*/	
    function DragDrop() 
	{
        var title = null, 
			...
			hintsList = {}, 
			questionHintsList = [],
			questionsSaveData = new SaveData();
        this.currentQuestion = null;
        this.currentPart = null;
        var superClient = null;
        var questionLevelHint = false;		
		...
		*/
        this.render = function () {
            ...
            $("<script/>", {
                type: "text/javascript",
                src: 'https://cdn.jsdelivr.net/jquery.ui.touch-punch/0.2.3/jquery.ui.touch-punch.js'
            }).appendTo("head");
            ...
            $.get(superClient.webContentFolder + assets.questions, function (data) 
			{
                $(data).find("question").each(function (q) 
				{
                    $(this).find("part").each(function (e) 
					{
						...
						{
                            hints[$(this).attr("id")] = {id: $(this).attr("id"), content: $(this).html()};
                            var target = $('.target').filter('[id=' + partId + ']');
							
                            target.parent().prepend('<a class="hintHat " alt="Click for hint" href="javascript();">Click for hint </a>');
							
                            target.parent().find('a').click(function () 
							{
                                var partId = $(this).parent().find('.target').attr('id');
                        ...
                        $(this).find("feedback").each(function (e) {
                            feedbacks[$(this).attr("id")] = {id: $(this).attr("id"), content: $(this).html(), pattern: $(this).attr("match")};
                        });
                        feedbackList[$(this).attr("id")] = feedbacks;
                    });
                    $(this).find("question_hint").each(function (e) {
                        questionHintsList.push({id: $(this).attr("id"), content: $(this).html()});
                    });
                });
		...
		*/		
        this.processLayout = function () 
		{
            $(".question").each(function () {
                var question = new Question($(this).attr('id'), $(this).find('.prompt').html());
                ...
                $(this).append('<div id="' + questionData.getId() + '_hints" class="hints"><p style="font-weight: bold;">' +
                        'Hints</p><div class="content"/><span class="xBtn">x</span><a class="next" href="javascript();">Next</a></div>');
                $('#' + questionData.getId() + '_hints').find('.next').click(function (e) {
                    e.preventDefault();
                    if (questionLevelHint) {
                        dndActivity.questionHint();
                    } else {
                        dndActivity.hint();
                    }
                });
                $('#' + questionData.getId() + '_hints').find('.xBtn').click(function (e) {
                    e.preventDefault();
                    $('#' + currentQuestion.getId() + '_hints').css('display', 'none');
		...
		*/		
        this.restoreSavedFile = function () 
		{
            ...
            if (typeof (superClient.currentAttempt) !== "undefined" && superClient.currentAttempt !== null && superClient.currentAttempt !== 'none') 
			{
                        ...
                        $(response).find('save_data').find('question').each(function (e) 
						{
                            ...
                            $(this).find('part').each(function (i) 
							{
                                ...
                                if ($(this).find('feedback')) {
                                    var feedback = {id: $(this).find('feedback').attr("id"), content: $(this).find('feedback').html()};
                                    partData.setFeedback(feedback);
                                }
                                if ($(this).find('hint')) {
                                    var hint = {id: $(this).find('hint').attr("id"), content: $(this).find('hint').html()};
                                ...
                                if (superClient.isCurrentAttemptCompleted()) {
                                    var borderColor = partCorrect ? 'style="border-color:#33aa33;"' :
                                            'style="border-color:#e75d36;"';
                                    var checkmark = partCorrect ? 'style="background-position: -18px 0px;"' :
                                            'style="background-position: -34px 0px;width: 12px;"';
                                    var pId = partData.getId();
                                    $('#' + questionData.getId() + '_feedback').append('<div><div class="circleBase type1" ' + borderColor + '>' +
                                            pId.substring(pId.lastIndexOf("_") + 1, pId.length) + ' </div><div class="checkMark"' + checkmark + '/><div style="display: inline-block;padding-left:35px;">' + partData.getFeedback().content + '</div></div>');
                                    $('.type1').show();
                                }
                            });
                            if (superClient.isCurrentAttemptCompleted()) {
                ...
                if (questionsData.hasOwnProperty(key)) 
				{
                    ...
                    for (var k in partsData) 
					{
                        if (partsData.hasOwnProperty(k)) 
						{
                            ...
                            var pId = pd.getId();
                            var borderColor = pd.getCorrect() ? 'style="border-color:#33aa33;"' :
                                    'style="border-color:#e75d36;"';
                            var checkmark = pd.getCorrect() ? 'style="background-position: -18px 0px;"' :
                                    'style="background-position: -34px 0px;width: 12px;"';
                            $('#' + qd.getId() + '_feedback').append('<div><div class="circleBase type1" ' + borderColor + '>' +
                                    pId.substring(pId.lastIndexOf("_") + 1, pId.length) + ' </div><div class="checkMark"' + checkmark + '/><div style="display: inline-block;padding-left:35px;">' + pd.getFeedback().content + '</div></div>');
                            $('.type1').show();
                        }
                    }
        ...
        this.makeActivityIntoDnD = function () 
		{
            ...
            $('.target').droppable({
                ...
                drop: function (event, ui) {
                    ...
                    var action = new ActionLog(CommonLogActionNames.START_STEP, superClient.sessionId, superClient.resourceId, superClient.activityGuid, "DND_ACTIVITY", superClient.timeZone);
		...
		*/		
        this.save = function () 
		{
            ...
            var action = new ActionLog(CommonLogActionNames.SAVE_ATTEMPT, superClient.sessionId, superClient.resourceId, superClient.activityGuid, "DND_ACTIVITY", superClient.timeZone);
            var supplement = new SupplementLog();
		...
		*/		
        this.scoreAttempt = function (attemptScore) 
		{
            ...
            var action = new ActionLog(CommonLogActionNames.SUBMIT_ATTEMPT, superClient.sessionId, superClient.resourceId, superClient.activityGuid, "DND_ACTIVITY", superClient.timeZone);
            var questionsData = questionsSaveData.getQuestionsData();
            for (var key in questionsData) {
                if (questionsData.hasOwnProperty(key)) {
		...
		*/		
        this.hint = function () 
		{
			...
			{
                var pd = questionsSaveData.getQuestionData(currentQuestion.getId()).getPartData(currentPart.getId());
                if (pd) 
				{
                    pd.setHint(hint);
                }
            }
            dndActivity.logHint(hint.content, currentQuestion.getId(), currentPart.getId());
            ...
            $('#' + currentQuestion.getId() + '_hints').find('.content').append('<div style="display: inline-block;"><div class="circleBase type1">' +
                    pId.substring(pId.lastIndexOf("_") + 1, pId.length) + ' </div><div style="display:inline-block;padding-left:25px;">' + hint.content + '</div><span class="xBtn">x</span></div>');
		...
		*
		*/		
        this.questionHint = function () 
		{
            console.log("questionHint()");
            ...
            if (typeof (currentQuestion.hintCnt) === "undefined" || currentQuestion.hintCnt === null) {
                currentQuestion.hintCnt = 0;
            }

            if (currentQuestion.hintCnt >= questionHintsList.length) {
                currentQuestion.hintCnt = 0;
            }
            var hint = questionHintsList[currentQuestion.hintCnt];
            currentQuestion.hintCnt++;
            ...
            if (questionsSaveData.getQuestionData(currentQuestion.getId())) {
                ...
                for (var key in pd) {
                    if (pd.hasOwnProperty(key)) {
                        pd[key].setHint(hint);
                        var pId = pd[key].getId();
                        dndActivity.logHint(hint.content, currentQuestion.getId(), pId);
                    }
                }
            }
            ...
            $('#' + currentQuestion.getId() + '_hints').find('.content').append('<div style="display: inline-block;"><div style="display:inline-block;padding-left:2px;">' + hint.content + '</div></div>');
            $('#' + currentQuestion.getId() + '_hints').css('display', 'block');
            $('.target').each(function () {
                $(this).parent().removeClass('hint_animate');
            });
            $('.target').each(function () {
                $(this).parent().add('hint_animate');
            });
            $('.type1').hide();
            questionLevelHint = true;
            //dndActivity.resizeIframe();
        };
		...
		*
		*/		
        this.logHint = function (content, questionId, partId) 
		{
