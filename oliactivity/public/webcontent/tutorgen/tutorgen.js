...
function generateProblemFromNode (xmlNode)
{
	...
	//var questionItems=shuffle (questionItems);
	...
	if (questionType=="multiple")
	{
		for (var j=0;j<questionItems.length;j++)
		{
			...
			if (aNode.nodeName=="input")
			{
				...
				if (aNode.attr)
				{
					if (aNode.attr.getNamedItem("shuffle"))
					{
						if (aNode.attr.getNamedItem("shuffle").nodeValue=="true")
						{
							shuffleAnswers=true;
						}
					}
				...
				{
					options=shuffle (aNode.childNodes);
				}	
...
{		
	...
	if (selected.length>0)
	{
		...
		for (var i=0;i<feedback.length;i++)
		{
			...
			testScore=Number(tSelection.score);
...
function startActivity ()
{
	...
	APIActivity.assignAdaptiveDriver (new OLITutorgenAdaptiveActivity (superClient,
	...
	for (var i=0;i<problemDefinitions.length;i++)
	{
		...
		{
			olidebug("File retrieve error: " + textStatus + ", " + errorThrown);
